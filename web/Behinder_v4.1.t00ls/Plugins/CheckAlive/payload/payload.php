...
function reachable($address){
    ...
    if (is_callable("exec"))
    {
        return $ping->ping("exec");
    }
    else if (strtoupper(substr(PHP_OS, 0, 3)) === "WIN" && is_callable("socket_create"))
    {
        return $ping->ping("socket");
    }
    else if (is_callable('posix_getuid')&&posix_getuid() == 0){
        return $ping->ping("socket");
    }
    else
    {
 ...
 */
public function __construct($host, $ttl = 255, $timeout = 5) {
  if (!isset($host)) {
...
private function pingExec() {
  ...
  if (!empty($output[1])) {
	...
	if ($response==0)
	{
		$response = preg_match("/ms/", $output[1], $matches);
	}
