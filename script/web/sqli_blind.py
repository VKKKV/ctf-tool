...
class BlindSQLi:
    def __init__(self, url, cookies=None, success_indicator="OK", error_indicator=None, timeout=5):
        self.url = url
    ...
    def _check(self, payload):
        ...
        try:
            response = requests.get(self.url, params=params, cookies=self.cookies, timeout=self.timeout)
            if self.error_indicator and self.error_indicator in response.text:
                print(f"
[!] Error detected in response: {self.error_indicator}")
                return False
...
def main():
    ...
    parser.add_argument("--method", choices=["linear", "binary"], default="binary", help="Extraction method")
