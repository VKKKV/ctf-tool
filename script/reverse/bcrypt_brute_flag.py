...
charset = [chr(i).encode("utf-8") for i in range(32, 127)]
...
for i, target_hash in enumerate(hashes):
    found = False
    print(f"[*] Cracking index {i} (Length {i + 1})... ", end="", flush=True)

    for char in charset:
        test_str = current_flag + char

        if bcrypt.checkpw(test_str, target_hash):
            current_flag = test_str
            print(f"Found! Current Flag: {current_flag.decode('utf-8')}")
            found = True
            break
